...
(define-datatype expressed-val expressed-val?
  ...
  (step-val (s step?))
  (point-val (p point?))
  ; proc-val for procedure support
  (proc-val (args list-of-symbols?) (body expr?) (env environment?))
  )
...
; following three functions are to extract from the proc-val
(define (proc-val->args p)
  (or (expressed-val? p) (invalid-args-exception "proc-val->args" "expressed-val?" p))
  (cases expressed-val p
    (proc-val (args body env) args)
    (else (invalid-args-exception "proc-val->args" "proc-val?" p))))

(define (proc-val->body p)
  (or (expressed-val? p) (invalid-args-exception "proc-val->body" "expressed-val?" p))
  (cases expressed-val p
    (proc-val (args body env) body)
    (else (invalid-args-exception "proc-val->body" "proc-val?" p))))

(define (proc-val->env p)
  (or (expressed-val? p) (invalid-args-exception "proc-val->env" "expressed-val?" p))
  (cases expressed-val p
    (proc-val (args body env) env)
    (else (invalid-args-exception "proc-val->env" "proc-val?" p))))
...
;=================================== proc ======================================
; datatype definition for procedures
(define-datatype proc proc?
  (procedure (args (list-of symbol?))
             (body expr?)
             (env environment?)))
; helper to locally bind arguments to parameter identifiers
(define (proc-env a p e)
  (if (null? a)
      e
      (proc-env (cdr a) (cdr p) (extend-env-wrapper (car a) (car p) e FINAL))))
