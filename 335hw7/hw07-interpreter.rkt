...
(define (value-of-program prog env)
  (cases program prog
    (a-program
     ...
     (let ((start-env (foldl value-of env var-exprs))) ; modify for leading var-exprs
     (andmap (lambda (ex) (value-of ex start-env))
...
(define (value-of-expr ex env)
  ...
  (cases expr ex
    ...
    (fun-expr
     (args body) (proc-val args body env))

    (fun-call-expr
     (f in-params)
     (letrec ((func (value-of f env))
              (args (proc-val->args func))
              (body (proc-val->body func))
              (in-env (proc-val->env  func))
              (param-vals (map (lambda (p) (value-of p env)) in-params)))
       (if (not (equal? (length in-params) (length args)))
           (raise (~a "arity mismatch. expected " (length args) " arguments, received " (length in-params)))
           (value-of body (proc-env args param-vals in-env)))))
...
(define (one-at-a-time-add var old-env)
  (cases var-expr var
    ...
    (fun-def-expr (id args body)
             (extend-env-wrapper id (value-of (fun-expr args body) old-env) old-env FINAL))))
...
(define (value-of-var v-ex env)
  ...
  (cases var-expr v-ex
    ...
    ; support for defined functions
    (fun-def-expr
     (id args body)
     (extend-env-wrapper id (value-of (fun-expr args body) env) env FINAL))
